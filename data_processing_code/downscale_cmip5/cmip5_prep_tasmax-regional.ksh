#!/bin/ksh
#First run in June 2013, modified for tasmax in Oct 2013, fixed buggy model simulations in March 2014
#Original script written by benjamin.poulter@lsce.ipsl.fr
#Modified June 2016 by Katie Renwick to downscale to 1-km for sage project
...
#Outputs
	...
	# Global uncorrected all ensembles at native resolution
...
fdirCMIP5hist='/mnt/lustrefs/work/katie.renwick/activescripts/wget/data/tasmax/historical/'
...
for rcp in ${rcpArray[@]}; do;
		...
		fdirCMIP5rcp='/mnt/lustrefs/work/katie.renwick/activescripts/wget/data/tasmax'/$rcp/

		#For output for final files
		#Default folder (so as not to delete home directory if not properly specified)
		echo "Making output directory"
		fdirOUT='/mnt/lustrefs/store/katie.renwick/Climate/Processed/Regional/wusa/CMIP5/tasmax'/$rcp/
		...
		ls $fdirCMIP5hist*_CCSM4_historical_r1i1p1_*nc ) )	#To select one model
		...
		ls $fdirCMIP5rcp*_CCSM4_*_r1i1p1_*nc) )	#To select one model
		...
		for fullName in ${modelListHist[@]}; do;
            ...
            if [ $startdate -lt 186101 ]; then
                ...
                if [ ! -f $fdirOUT/"tasmax_Amon_"$model"_historical_"$ensemble"_1861-2005.nc" ]; then
                    #########################
                    #Merge the historical GCM run time slices for the same ensemble
                    echo $fullName
                    cdo -s mergetime $fdirCMIP5hist/"tasmax_Amon_"$model"_historical_"$ensemble"_"* $fdirOUT/"tasmax_Amon_"$model"_historical_"$ensemble"_allyears.nc"
                    ...
                    if [ $enddate = "200511" -o $model = "HadGEM2-ES" ]; then
                        ...
                        cdo -s seldate,1861-01-01,2005-11-31 $fdirOUT/"tasmax_Amon_"$model"_historical_"$ensemble"_allyears.nc" $fdirOUT/"tasmax_Amon_"$model"_historical_"$ensemble"_1861-2005.nc"
                    elif [ $enddate != "200511" ]; then
                        #Clip the merged GCM runs to 1861-2005 (December)
                        cdo -s seldate,1861-01-01,2005-12-31 $fdirOUT/"tasmax_Amon_"$model"_historical_"$ensemble"_allyears.nc" $fdirOUT/"tasmax_Amon_"$model"_historical_"$ensemble"_1861-2005.nc"
                    fi

                    #Clean up
                    rm -f $fdirOUT/"tasmax_Amon_"$model"_historical_"$ensemble"_allyears.nc"
                fi
            fi
		done
		...
		for fullName in ${modelListRcp[@]}; do;
			...
			if [ ! -f $fdirOUT/"tasmax_Amon_"$model"_"$rcp"_"$ensemble"_2006-2099.nc" ]; then
				#########################
                ...
                if [[ $model != "HadGEM2-AO" ]]; then
                    ...
                    cdo -s mergetime $fdirCMIP5rcp/"tasmax_Amon_"$model"_"$rcp"_"$ensemble"_"* $fdirOUT/"tasmax_Amon_"$model"_"$rcp"_"$ensemble"_allyears.nc"
                elif [ $model = "HadGEM2-AO" ]; then
                    echo Special Case: $fullName
                    cdo -s mergetime $fdirCMIP5rcp/"tasmax_Amon_"$model"_"$rcp"_"$ensemble"_"*209912* $fdirOUT/"tasmax_Amon_"$model"_"$rcp"_"$ensemble"_allyears.nc"
                elif [ $model = "CCSM4" -a $rcp = "rcp26" ]; then
                    echo Special Case: $fullName
                    cdo -s mergetime $fdirCMIP5rcp/"tasmax_Amon_"$model"_"$rcp"_"$ensemble"_"*200601* $fdirOUT/"tasmax_Amon_"$model"_"$rcp"_"$ensemble"_allyears.nc"
                    ...
                    echo Special Case: $fullName
                    cdo -s mergetime $fdirCMIP5rcp/"tasmax_Amon_"$model"_"$rcp"_"$ensemble"_"*200501* $fdirOUT/"tasmax_Amon_"$model"_"$rcp"_"$ensemble"_allyears.nc"
                fi
				...
				if [ $startdate != "200512" ]; then
					...
					cdo -s seldate,2006-01-01,2099-12-31 $fdirOUT/"tasmax_Amon_"$model"_"$rcp"_"$ensemble"_allyears.nc" $fdirOUT/"tasmax_Amon_"$model"_"$rcp"_"$ensemble"_2006-2099.nc"
				fi
				if [ $startdate = "200512" ]; then
					#Clip the merged GCM runs to 2006-2099
					cdo -s seldate,2005-12-01,2099-12-31 $fdirOUT/"tasmax_Amon_"$model"_"$rcp"_"$ensemble"_allyears.nc" $fdirOUT/"tasmax_Amon_"$model"_"$rcp"_"$ensemble"_2006-2099.nc"
				fi
				
				#Clean up
				rm -f $fdirOUT/"tasmax_Amon_"$model"_"$rcp"_"$ensemble"_allyears.nc"
			fi
		done
		...
		for fullName in ${modelListHistClean[@]}; do;
            ...
            if [ -f $fdirOUT/"tasmax_Amon_"$model"_"$rcp"_"$ensemble"_2006-2099.nc" ]; then
                cdo mergetime $fdirOUT/"tasmax_Amon_"$model"_historical_"$ensemble"_1861-2005.nc" $fdirOUT/"tasmax_Amon_"$model"_"$rcp"_"$ensemble"_2006-2099.nc" $fdirOUT/"tasmax_Amon_"$model"_"$rcp"_"$ensemble"_1861-2099_K.nc"
                cdo -s subc,273.15 $fdirOUT/"tasmax_Amon_"$model"_"$rcp"_"$ensemble"_1861-2099_K.nc" $fdirOUT/"tasmax_Amon_"$model"_"$rcp"_"$ensemble"_1861-2099.nc"
            else
                echo "No " $fdirOUT/"tasmax_Amon_"$model"_"$rcp"_"$ensemble"_2006-2099.nc" " exists for the historical merge"
            fi
            
            #Clean up
            rm -rf $fdirOUT/"tasmax_Amon_"$model"_historical_"$ensemble"_1861-2005.nc"		#Remove old historical
            rm -rf $fdirOUT/"tasmax_Amon_"$model"_"$rcp"_"$ensemble"_2006-2099.nc"			#Remove old RCP
            rm -rf $fdirOUT/"tasmax_Amon_"$model"_"$rcp"_"$ensemble"_1861-2099_K.nc"       #Remove kelvin file
		done
        ...
        if [ $rcp = "rcp26" -o $rcp = "rcp45" -o $rcp = "rcp60" -o $rcp = "rcp85" ]; then
            rm -f $fdirOUT/"tasmax_Amon_MIROC5_"$rcp"_r4i1p1_1861-2099.nc"
            rm -f $fdirOUT/"tasmax_Amon_MIROC5_"$rcp"_r5i1p1_1861-2099.nc"
        fi
        #Remove HadCM3 because it ends in 2035
        if [ $rcp = "rcp45" ]; then
            rm -f $fdirOUT/"tasmax_Amon_HadCM3_"*
        fi        #Remove file because it ends in November 2099, not December
        if [ $rcp = "rcp60" ]; then
            rm -f $fdirOUT/"tasmax_Amon_HadGEM2-ES_rcp60_r1i1p1_1861-2099.nc"
        fi
        #Remove corrupt files
        if [ $rcp = "rcp85" ]; then
            #rm -f $fdirOUT/"tasmax_Amon_CESM1-CAM5_rcp85_r1i1p1_1861-2099.nc"
            rm -f $fdirOUT/"tasmax_Amon_CESM1-CAM5_rcp85_r3i1p1_1861-2099.nc"
            rm -f $fdirOUT/"tasmax_Amon_FIO-ESM_rcp85_r1i1p1_1861-2099.nc"
        fi
        #Fix repeated 2099 December month for just a single HadGEM2-ES run
        if [ $rcp = "rcp85" -a -e $fdirOUT/"tasmax_Amon_HadGEM2-ES_rcp85_r1i1p1_1861-2099.nc" ]; then
            cdo -s splitsel,2868 $fdirOUT/"tasmax_Amon_HadGEM2-ES_rcp85_r1i1p1_1861-2099.nc" tmp
            rm -f $fdirOUT/"tasmax_Amon_HadGEM2-ES_rcp85_r1i1p1_1861-2099.nc"
            cp tmp000000.nc $fdirOUT/"tasmax_Amon_"$model"_"$rcp"_"$ensemble"_1861-2099.nc"
            rm -f tmp*
        fi
		...
		modelNoHistorical=( $( ls $fdirOUT/"tasmax_Amon_"*"_"*"_"*"_2006-2099.nc" ) )
...
ls $fdirOUT*$rcp*1861-2099.nc
		...
		for fullName in ${modelListEns[@]}; do;
			...
			cdo -s remapbil,$fdirGRIDglobal/cru_05deg $fdirOUT/"tasmax_Amon_"$model"_"$rcp"_"$ensemble"_1861-2099.nc" $fdirOUT/"tasmax_Amon_"$model"_"$rcp"_"$ensemble"_1861-2099_05deg.nc"
			
			#Clip to the wusa
			cdo -s sellonlatbox,-124.25,-102.25,31.75,49 $fdirOUT/"tasmax_Amon_"$model"_"$rcp"_"$ensemble"_1861-2099_05deg.nc" $fdirOUT/"tasmax_Amon_"$model"_"$rcp"_"$ensemble"_1861-2099-wusa.nc"
			
			#Remove global version
			rm -f $fdirOUT/"tasmax_Amon_"$model"_"$rcp"_"$ensemble"_1861-2099.nc"
			rm -f $fdirOUT/"tasmax_Amon_"$model"_"$rcp"_"$ensemble"_1861-2099_05deg.nc" 
		done
		...
		for fullName in ${modelListEnswusa[@]}; do;
			...
			if [ ! -f $fdirOUT/"tasmax_Amon_"$model"_"$rcp"_1861-2099_global-cor.nc" ]; then	
				#########################
                ...
                if [ $model = "GFDL-CM3" ]; then
                    cdo -s splitcode $fdirOUT/"tasmax_Amon_"$model"_"$rcp"_"$ensemble"_1861-2099-wusa.nc" $fdirOUT/"tasmax_Amon_"$model"_"$rcp"_"$ensemble"_1861-2099"
                    mv $fdirOUT/"tasmax_Amon_"$model"_"$rcp"_"$ensemble"_1861-2099-01.nc"* $fdirOUT/"tasmax_Amon_"$model"_"$rcp"_"$ensemble"_1861-2099-wusa.nc"
                    rm -f $fdirOUT/"tasmax_Amon_"$model"_"$rcp"_"$ensemble"_1861-2099-02.nc"*
                fi

				#########################
				#Regrid the 0.5 degree GCM climate data to the grid Daymet 1KM
				cdo -s remapbil,$fdirGRID/wusa_1km $fdirOUT/"tasmax_Amon_"$model"_"$rcp"_"$ensemble"_1861-2099-wusa.nc" $fdirOUT/"tasmax_Amon_"$model"_"$rcp"_"$ensemble"_1861-2099-1km.nc"

                #########################
				#Bias correct the GCM data
				#Calculate monthly means of GCM simulations for 1961-1990 observation period
				cdo -s seldate,1980-01-01,1999-12-31 $fdirOUT/"tasmax_Amon_"$model"_"$rcp"_"$ensemble"_1861-2099-1km.nc" $fdirOUT/"tasmax_Amon_"$model"_"$rcp"_"$ensemble"_1981-1999-1km.nc"
				cdo -s ymonmean $fdirOUT/"tasmax_Amon_"$model"_"$rcp"_"$ensemble"_1981-1999-1km.nc" $fdirOUT/"tasmax_Amon_"$model"_"$rcp"_"$ensemble"_1981-1999-1km-ymonmean.nc"
				...
				cdo -s ymonadd $fdirOUT/"tasmax_Amon_"$model"_"$rcp"_"$ensemble"_1861-2099-1km.nc" $fdirOUT/"tasmax_Amon_"$model"_"$rcp"_"$ensemble"_1981-1999-1km-cf.nc" $fdirOUT/"tasmax_Amon_"$model"_"$rcp"_"$ensemble"_1861-2099-1km-cor_global.nc"
				...
				if [ $fullName = ${modelListConv[${#modelListConv[@]}-1]} ]; then
					...
					if [ $ensembleLength -gt 1 ]; then
						...
						cdo -s ensmean $fdirOUT/"tasmax_Amon_"$model"_"$rcp"_"*"_1861-2099-1km.nc" $fdirOUT/"tasmax_Amon_"$model"_"$rcp"_1861-2099_globalEns-uncor.nc"

						#Calculate the ensemble mean for corrected
						cdo -s ensmean $fdirOUT/"tasmax_Amon_"$model"_"$rcp"_"*"_1861-2099-1km-cor_global.nc" $fdirOUT/"tasmax_Amon_"$model"_"$rcp"_1861-2099_globalEns-cor.nc"
					else
						########################
						#Copy the ensemble for uncorrected - can select all ensembles because there is just one... but keep an eye for problems
						cp $fdirOUT/"tasmax_Amon_"$model"_"$rcp"_"*"_1861-2099-1km.nc" $fdirOUT/"tasmax_Amon_"$model"_"$rcp"_1861-2099_globalEns-uncor.nc"

						#Copy the ensemble for corrected - can select all ensembles because there is just one... but keep an eye for problems
						cp $fdirOUT/"tasmax_Amon_"$model"_"$rcp"_"*"_1861-2099-1km-cor_global.nc" $fdirOUT/"tasmax_Amon_"$model"_"$rcp"_1861-2099_globalEns-cor.nc"
					fi
					
					#########################
					#Back up the first ensemble member ( should be r1 ... with r10 & higher selected after lower ensemble r members)
					cp $fdirOUT/"tasmax_Amon_"$model"_"$rcp"_"$firstEnsemble"_1861-2099-1km-cor_global.nc" $fdirOUT/"tasmax_Amon_"$model"_"$rcp"_1861-2099_"$firstEnsemble"_global-cor.nc"
					
					#########################
					#Clean up
					#rm -rf $fdirOUT/"tasmax_Amon_"$model"_"$rcp"_"$ensemble"_1861-2099.nc"		#Keep this file to check ensemble variability
					rm -rf $fdirOUT/"tasmax_Amon_"$model"_"$rcp"_"$ensemble"_1861-2099-wusa.nc"		#Remove the clipped 0.5 degree CMIP5 file
					rm -rf $fdirOUT/"tasmax_Amon_"$model"_"$rcp"_"*"_1981-1999-1km.nc"				#Remove the non-bias corrected 1km CMIP5 file
					rm -rf $fdirOUT/"tasmax_Amon_"$model"_"$rcp"_"*"_1981-1999-1km-ymonmean.nc"	#Remove the 1km monthly means for the CMIP5 file
					rm -rf $fdirOUT/"tasmax_Amon_"$model"_"$rcp"_"*"_1981-1999-1km-cf.nc"			#Remove the correction factor file
					rm -rf $fdirOUT/"tasmax_Amon_"$model"_"$rcp"_"*"_1861-2099-1km.nc"				#Remove all the uncorrected 1km ensembles but keep orginal res. ensemble
					rm -rf $fdirOUT/"tasmax_Amon_"$model"_"$rcp"_"*"_1861-2099-1km-cor_global.nc"	#Remove all the corrected ensembles but keep first ensemble
